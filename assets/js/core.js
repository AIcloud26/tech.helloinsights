...
HI.footer=function(){var el=document.querySelector("[data-footer-topics]");if(!el)return;el.innerHTML=SITE.categories.map(function(c){return '<li><a href="/category.html?cat='+encodeURIComponent(c.id)+'">'+HI.esc(c.name)+'</a></li>'}).join("")};
HI.topics=function(){var el=document.querySelector("[data-topic-pills]");if(!el)return;el.innerHTML=SITE.categories.map(function(c){return '<a href="/category.html?cat='+encodeURIComponent(c.id)+'">'+HI.esc(c.short)+'</a>'}).join("")};
